...
def format_timestamp(timestamp: float | datetime, offset_timezone: timedelta, format: str = "%Y-%m-%d %H:%M:%S"):
    """
    格式化时间戳为指定时区的日期时间字符串
...
if __name__ == "__main__":
    # 示例1: UTC时间戳0在UTC时区
    print(format_timestamp(0, timedelta(hours=0)))  # 输出: 1970-01-01 00:00:00
    
    # 示例2: UTC时间戳0在东八区
    print(format_timestamp(0, timedelta(hours=8)))  # 输出: 1970-01-01 08:00:00
    
    # 示例3: UTC时间戳0在纽约时区(西五区)
    print(format_timestamp(0, timedelta(hours=-5)))  # 输出: 1969-12-31 19:00:00
    
    # 示例4: 小数时区(印度时区UTC+5.5)
    print(format_timestamp(0, timedelta(hours=5, minutes=30)))  # 输出: 1970-01-01 05:30:00
    
    # 示例5: 带毫秒的时间戳
    ts = 1691417235.678  # 2023-08-07 12:07:15.678 UTC
    print(format_timestamp(ts, timedelta(hours=8), "%Y-%m-%d %H:%M:%S.%f"))  # 输出: 2023-08-07 20:07:15.678