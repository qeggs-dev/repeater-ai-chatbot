...
.content-container {
    ...
    border-radius: 6px 6px 24px 24px; /* 只保留下边圆角 */
    padding: 40px 50px;
    box-shadow: 
...
.title-bar {
    ...
    z-index: 2;
    box-shadow: 
        0 3px 10px rgba(52, 152, 219, 0.25),
...
.content-container mark {
    background: linear-gradient(120deg, rgba(241, 196, 15, 0.3) 0%, rgba(241, 196, 15, 0.1) 100%);
    color: var(--gray-900);
    padding: 0.1em 0.3em;
    border-radius: 3px;
    display: inline;
...
/* 长文本和URL自动换行处理 - 更智能的换行策略 */
.content-container .long-text,
.content-container .long-url,
.content-container .break-words {
...
.content-container .text-success {
    color: var(--success);
}
...
.content-container .text-danger {
    color: var(--danger);
}

.content-container .text-primary {
    color: var(--primary);
}

/* === 文本布局优化 === */

/* 确保内容容器内的所有元素都能充分利用宽度 */
.content-container > * {
    width: 100%;
    box-sizing: border-box;
}
...
/* 列表项内的内容优化 */
.content-container li > *:not(ul):not(ol) {
    display: inline;
}
