import asyncio
from typing import Any
from loguru import logger
from pydantic import ValidationError
from ..Data_Manager import UserConfigManager
from ._exceptions import *
from ._object import UserConfigs
from ..Global_Config_Manager import ConfigManager as GlobalConfigManager

class ConfigManager(UserConfigManager):
    # 全局配置缓存
    _global_config_cache:dict[str, UserConfigs] = {}
    
    def __init__(
            self,
            cache: bool = True,
            use_global_cache: bool = False,
            downgrade_wait_time: float | None = None,
            debonce_save_wait_time: float | None = None
        ):
        super().__init__()
        self._cache:dict[str, UserConfigs] = {}

        self._cache_switch = cache
        self._use_global_cache = use_global_cache

        self._downgrade_tasks: dict[str, asyncio.Task] = {}
        self._downgrade_wait_time: float = downgrade_wait_time or GlobalConfigManager.get_configs().user_config_cache.downgrade_wait_time

        self._debonce_save_tasks: dict[str, asyncio.Task] = {}
        self._debonce_save_wait_time: float = debonce_save_wait_time or GlobalConfigManager.get_configs().user_config_cache.debounce_save_wait_time

        self._lock = asyncio.Lock()

    async def _get_cache(self) -> dict[str, UserConfigs]:
        if self._use_global_cache:
            return self._global_config_cache
        else:
            return self._cache
    
    async def load(self, user_id: str) -> UserConfigs:
        """
        获取用户配置
        
        :param user_id: 用户ID
        :return: 用户配置
        """
        cache = await self._get_cache()

        if self._cache_switch:
            async with self._lock:
                if user_id in cache:
                    logger.info("Get user config form cache", user_id = user_id)
                    config = cache[user_id]
                else:
                    config = await self._load_configs(user_id)
                    cache[user_id] = config
                    logger.info("Get user config from storage and save to cache", user_id = user_id)
                
                # 如果已有任务，先取消
                if user_id in self._downgrade_tasks and not self._downgrade_tasks[user_id].done():
                    self._downgrade_tasks[user_id].cancel()
                task = asyncio.create_task(
                    self._wait_and_downgrade(user_id=user_id, wait_time=self._downgrade_wait_time)
                )
                logger.info(f"Create user config downgrade task", user_id = user_id)
                # 任务完成自动删除
                task.add_done_callback(lambda _, id=user_id: self._downgrade_tasks.pop(id, None))
                self._downgrade_tasks[user_id] = task
            return config
        else:
            logger.info("Get user config from storage", user_id = user_id)
            return await self._load_configs(user_id)
    
    async def save(self, user_id: str, configs: UserConfigs) -> None:
        """
        保存用户配置

        :param user_id: 用户ID
        :param configs: 用户配置
        :return: None
        """
        cache = await self._get_cache()

        if self._cache_switch:
            if user_id in cache:
                if isinstance(configs, UserConfigs):
                    async with self._lock:
                        logger.info("Save user config to cache", user_id = user_id)
                        cache[user_id] = configs

                        if user_id in cache and (user_id in self._debonce_save_tasks and not self._debonce_save_tasks[user_id].done()):
                            self._debonce_save_tasks[user_id].cancel()
                        task = asyncio.create_task(
                            self._wait_and_save(user_id, self._debonce_save_wait_time)
                        )
                        logger.info(f"Create user config save task", user_id = user_id)
                        task.add_done_callback(lambda _, id=user_id: self._debonce_save_tasks.pop(id, None))
                        self._debonce_save_tasks[user_id] = task
                else:
                    raise TypeError("user configs must be Configs object")
        else:
            logger.info("Save user config to storage", user_id = user_id)
            await self._save_configs(user_id, configs)
    
    async def _wait_and_downgrade(self, user_id: str, wait_time: float = 5) -> None:
        """
        等待并降级用户配置

        :param user_id: 用户ID
        :param wait_time: 等待时间
        :return: None
        """
        try:
            await asyncio.sleep(wait_time)
            async with self._lock:
                if user_id in self._debonce_save_tasks and not self._debonce_save_tasks[user_id].done():
                    logger.warning("User config Save task is running, downgrade config cancelled", user_id = user_id)
                    return
                cache = await self._get_cache()
                if user_id in cache:
                    logger.info("Downgrade user config", user_id = user_id)
                    del cache[user_id]
        except asyncio.CancelledError:
            logger.info("User config downgrade task cancelled", user_id = user_id)
    
    async def _wait_and_save(self, user_id: str, wait_time: float = 5) -> None:
        """
        等待并保存用户配置

        :param user_id: 用户ID
        :param wait_time: 等待时间
        :return: None
        """
        try:
            await asyncio.sleep(wait_time)
            async with self._lock:
                cache: dict[str, UserConfigs] = await self._get_cache()
                if user_id in cache:
                    logger.info("Save user config", user_id = user_id)
                    await self._save_configs(user_id, cache[user_id])
                    del cache[user_id]
        except asyncio.CancelledError:
            logger.info("User config save task cancelled", user_id = user_id)
    
    async def delete(self, user_id: str) -> None:
        """
        删除用户配置

        :param user_id: 用户ID
        :return: None
        """
        async with self._lock:
            cache = await self._get_cache()
            if user_id in cache:
                del cache[user_id]
            await super().delete(user_id)
            logger.info("Delete user config", user_id = user_id)
    
    async def clear_cache(self) -> None:
        """
        清除缓存
        """
        (await self._get_cache()).clear()
    
    async def force_write(self, user_id: str, configs: UserConfigs) -> None:
        """
        强制写入配置

        :param user_id: 用户ID
        :param configs: 用户配置
        :return: None
        """
        async with self._lock:
            cache = await self._get_cache()
            await self._save_configs(user_id, configs)
            cache[user_id] = configs
            logger.info("Force write user config", user_id = user_id)

    async def save_all(self, clear_all_tasks: bool = True) -> None:
        """
        保存所有用户配置

        :param clear_all_tasks: 是否清除所有防抖任务
        :return: None
        """
        async with self._lock:
            cache = await self._get_cache()

            for user_id, configs in cache.items():
                await self._save_configs(user_id, configs)
                if clear_all_tasks:
                    self._downgrade_tasks[user_id].cancel()
                    self._debonce_save_tasks[user_id].cancel()
            logger.info(f"Saved {len(cache)} user config", user_id = user_id)
            cache.clear()
    
    async def get_all_configs(self):
        """
        获取所有用户配置

        :return: 所有用户的配置数据
        """
        async with self._lock:
            configs: dict[str, UserConfigs] = {}
            cache = await self._get_cache()

            userlist = await self.get_all_user_id()
            for user_id in userlist:
                if user_id in cache:
                    configs[user_id] = cache[user_id]
                else:
                    configs[user_id] = await self._load_configs(user_id)
            logger.info(f"Loaded {len(configs)} user configs")
            return configs
    
    async def _load_configs(self, user_id: str) -> UserConfigs:
        """
        读取用户配置

        :param user_id: 用户ID
        :return: 用户配置数据
        """
        try:
            return UserConfigs(
                **await super().load(
                    user_id
                )
            )
        except ValidationError as e:
            errors = e.errors()
            text_buffer:list[str] = []
            for error in errors:
                text_buffer.append(
                    f"{error['loc'][0]}: {error['msg']}"
                )
            raise Exception("\n".join(text_buffer))
    
    async def _save_configs(self, user_id: str, configs: UserConfigs):
        """
        写入用户配置

        :param user_id: 用户ID
        :param configs: 配置数据
        """
        await super().save(
            user_id, configs.model_dump(exclude_none=True)
        )
    
    
    async def __aenter__(self):
        return self 

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        await self.save_all()